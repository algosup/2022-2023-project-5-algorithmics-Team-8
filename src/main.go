...
func containsinfloat(slice []float64, element float64) bool {
	for _, e := range slice {
		if e == element {
			return true
		}
...
func main() {
	...
	if file, err = os.Open("config_file_path"); err == nil {
		defer file.Close()
		...
		for scanner.Scan() {
			...
			if line == "" || line[0] == '!' || line[0] == '\r' {
				...
				pos2 := strings.Index(line, ";") + pos1 + 1
				...
				if pos1 != -1 && pos2 != -1 {
					...
					v, _ := strconv.ParseFloat(line[pos1+1:pos2], 64)
					c, _ := strconv.ParseFloat(line[pos1+1:pos2], 64)
					...
					if tank.volume <= 0 || tank.capacity > 1000 {
						log.Fatalf("Error: Invalid tank volume for tank %s. Volume should be between 0 and 1000.", tank.number)
						return
					}

					pos3 := strings.Index(line, ";") + pos2 + 1
					if pos3 != -1 {
						log.Fatalf("Error: Multiple tanks quantities specified for tank %s. Only the first quantity will be considered.", tank.number)
						return
					}

					wineTank := line[pos2+1:]
					var wine string
					found := false

					if (wineTank == "champagne") || (wineTank == "/") || (wineTank == "Champagne") {
						wine = wineTank
						found = true
					} else {
						found = false
					...
					if !found {
						log.Fatal("Error: Unknown wine type for the tank %s", wineTank)
						return
					}

					w, _ := strconv.ParseFloat(wine, 64)

					tank.winesUsed = append(tank.winesUsed, w)
		...
		requiredVolumes := make([]float64, len(wines))
		for i := 0; i < len(wines); i++ {
			requiredVolumes[i] = totalVolume * percentages[i] / 100
		}
		...
		for i := 0; i < len(wines); i++ {
			remainingVolume := requiredVolumes[i]
			...
			if remainingVolume > totalAvailableVolume {
				fmt.Println("The volume needed is more than the volume you have inside your empty tanks")
				return
			}
		...
		for i := 0; i < len(wines); i++ {
			var totalTanksVolume float64 = 0.0
			var requiredVolumes float64 = 0.0

			for j := 0; j < len(tanks); j++ {
				if contains(tanks[j].winesStocked, wines[i]) {
					totalTanksVolume += tanks[j].volume
				}
			}
			for j := 0; j < len(tanksToUse[i]); j++ {
				var tankIndex int = tanksToUse[i][j]
				requiredVolumes += tanksVolumes[tankIndex][i]
				remainingVolumes[i].volume = requiredVolumes - totalTanksVolume
			}
			fmt.Println("required volume for " + wines[i] + " is " + fmt.Sprintf("%f", requiredVolumes) + "hl")

			if totalTanksVolume < requiredVolumes {
				log.Fatal("Error, the volume of " + wines[i] + " is not enough to fill a tank")
				return
			}

			var remainingVolume float64 = totalTanksVolume - requiredVolumes

			if remainingVolume > 0 {
				fmt.Println("Impossible to have " + wines[i] + " you need to empty " + fmt.Sprint("%f", (-remainingVolume)) + "hl more")
			} else {
			...
			for i := 0; i < len(wines); i++ {
				...
				for j := 0; j < len(tanksToUse[i]); j++ {
					...
					if !tanksAlreadyWritten[tankIndex] {
						file.Write([]byte("Tank: " + tanks[tankIndex].number + "\n"))
						...
						file.Write([]byte("Filling " + tanks[tankIndex].number + " (" + wines[i] + ")\n"))
						...
						for (remainingVolume > 0) && (k < len(tanksToUse[i])) {
							if tanks[k].winesStocked[0] == wines[i] && tanks[k].volume > 0 && tanks[k].volume > 0.001 {
								...
								file.Write([]byte(string("Transfering " + fmt.Sprintf("%f", transferVolume) + "hl from " + tanks[k].number + "\n")))
			...
			for i := 0; i < len(tanks); i++ {
				...
				if tank.winesStocked[0] != "/" && tank.volume > 0 && tank.winesStocked[0] != "Champagne" && tank.winesStocked[0] != "champagne" && tank.winesStocked[0] != "CHAMPAGNE" && !tank.transferred {
					fmt.Println("Tank " + tank.number + " still has " + fmt.Sprintf("%f", tank.volume) + "hl of " + tank.winesStocked[0])

					var remainingVolume float64 = 0.0
					for j := 0; j < len(wines); j++ {
						if remainingVolumes[j].wine == tank.winesStocked[0] && remainingVolumes[j].tankID == i {
					...
					if bestTankIndex != -1 {
						tanks[bestTankIndex].winesStocked[0] = tank.winesStocked[0]
						file.Write([]byte("Transfering " + fmt.Sprintf("%F", tanks[bestTankIndex].volume) + "hl from " + tank.winesStocked[0] + " to " + tanks[bestTankIndex].number + "\n"))
						file.Write([]byte("\n"))
						tank.winesStocked[0] = "/"
				...
				for index := 0; index < bestCombinationLen; index++ {
					...
					if tanks[index].capacity >= 0.1 && !ok {
						...
						if volumeOfTransfert >= 0.1 && tank.volume >= 0.1 {
							...
							file.Write([]byte("Transfer of " + fmt.Sprintf("%f", volumeOfTransfert) + "hl of " + tanks[index].winesStocked[0] + " from tank " + tank.number + " to tank " + tanks[index].number + "\n"))
			...
			for i := 0; i < len(tanks); i++ {
				...
				if tank.winesStocked[0] == "/" {
					...
					fmt.Println("Tank " + tank.number + ": " + fmt.Sprintf("%f", volume) + "hl of " + tank.winesStocked[0])
				} else {
					fmt.Println("Tank " + tank.number + ": " + fmt.Sprintf("%f", tank.volume) + "hl of " + tank.winesStocked[0])
				}
			}
			file.Write([]byte("\n"))
			file.Write([]byte("EN\n"))
			...
			for i := 0; i < len(tanks); i++ {
				...
				if tank.volume > 0 && tank.volume < tank.volume {
					file.Write([]byte("Tank " + tank.number + ": " + fmt.Sprintf("%f", tank.volume) + "hl of " + tank.winesStocked[0] + "\n"))
				}
			}
			...
			for i := 0; i < len(tanks); i++ {
				...
				tank := &tanks[i]
				if tank.winesStocked[0] == "Champagne" || tank.winesStocked[0] == "Champagne" {
					file.Write([]byte("Tank " + tank.number + ": " + fmt.Sprintf("%f", tank.volume) + "hl of " + tank.winesStocked[0] + "\n"))
				}
			}
			file.Write([]byte("\n"))
